...
//! `reed_solomon_erasure` on the same shard layout. The `node run.mjs` driver
//! times both. `reconstruct` uses the data-only path (`reconstruct_data`) on
//! each, matching the native cross-crate benches in `comparisons/`.
